...
export const CollectionCard = ({ collection }: { collection: CatalogCollection }) => {
  const fileCount = collection.items.reduce((sum, item) => sum + item.files.length, 0);
  ...
  return (
    ...
    >
      <h2 className="text-lg font-semibold text-primary-900">{collection.id}</h2>
      <p className="mt-1 text-sm text-primary-600">{collection.name}</p>
      {collection.description && <p className="mt-1 line-clamp-2 text-xs text-primary-400">{collection.description}</p>}
